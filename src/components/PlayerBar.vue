<template>
  <div class="player-bar">
    <div class="audio-avatar">
      <img class="avatar-image" v-if="currentSong.image" :src="currentSong.image" alt="lucymusic">
      <div v-else><svg t="1615880982818" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2830"><path d="M513 511.8m-497 0a497 497 0 1 0 994 0 497 497 0 1 0-994 0Z" fill="#A99BF9" p-id="2831"></path><path d="M513 176.6c-185.1 0-335.2 150.1-335.2 335.2S327.9 847 513 847s335.2-150.1 335.2-335.2S698.1 176.6 513 176.6z m0 377.7c-23.5 0-42.5-19-42.5-42.5s19-42.5 42.5-42.5 42.5 19 42.5 42.5-19 42.5-42.5 42.5z" fill="#657182" p-id="2832"></path><path d="M513 854.6c-91.6 0-177.7-35.7-242.4-100.4-64.8-64.8-100.4-150.9-100.4-242.4 0-91.6 35.7-177.7 100.4-242.4S421.4 169 513 169s177.7 35.7 242.4 100.4c64.8 64.8 100.4 150.9 100.4 242.4 0 91.6-35.7 177.7-100.4 242.4C690.7 819 604.6 854.6 513 854.6z m0-670.3c-87.5 0-169.7 34.1-231.6 95.9-61.9 61.9-95.9 144.1-95.9 231.6s34.1 169.7 95.9 231.6c61.9 61.9 144.1 95.9 231.6 95.9s169.7-34.1 231.6-95.9c61.9-61.9 95.9-144.1 95.9-231.6s-34.1-169.7-95.9-231.6c-61.9-61.9-144.1-95.9-231.6-95.9z m0 377.7c-27.7 0-50.2-22.5-50.2-50.2 0-27.7 22.5-50.2 50.2-50.2 27.7 0 50.2 22.5 50.2 50.2 0 27.7-22.5 50.2-50.2 50.2z m0-85.1c-19.2 0-34.9 15.7-34.9 34.9s15.7 34.9 34.9 34.9 34.9-15.7 34.9-34.9-15.7-34.9-34.9-34.9z" fill="#100F11" p-id="2833"></path><path d="M513 383.8c-70.7 0-128 57.3-128 128s57.3 128 128 128 128-57.3 128-128-57.3-128-128-128z m0 170.5c-23.5 0-42.5-19-42.5-42.5s19-42.5 42.5-42.5 42.5 19 42.5 42.5-19 42.5-42.5 42.5z" fill="#E45544" p-id="2834"></path><path d="M513 647.4c-74.8 0-135.6-60.8-135.6-135.6S438.2 376.2 513 376.2 648.6 437 648.6 511.8 587.8 647.4 513 647.4z m0-255.9c-66.3 0-120.3 54-120.3 120.3s54 120.3 120.3 120.3 120.3-54 120.3-120.3-54-120.3-120.3-120.3z m0 170.5c-27.7 0-50.2-22.5-50.2-50.2s22.5-50.2 50.2-50.2c27.7 0 50.2 22.5 50.2 50.2S540.7 562 513 562z m0-85.1c-19.2 0-34.9 15.7-34.9 34.9s15.7 34.9 34.9 34.9 34.9-15.7 34.9-34.9-15.7-34.9-34.9-34.9z" fill="#0F0E10" p-id="2835"></path><path d="M789.1 414.8c-3.2 0-6.1-2-7.2-5.1-13.7-38.6-35.4-73-64.6-102.2-29.2-29.2-63.6-50.9-102.2-64.6-4-1.4-6.1-5.8-4.7-9.8s5.8-6.1 9.8-4.7c40.8 14.4 77.1 37.4 107.9 68.2 30.8 30.8 53.8 67.1 68.2 107.9 1.4 4-0.7 8.4-4.7 9.8-0.8 0.3-1.6 0.5-2.5 0.5zM739.2 424c-3.1 0-5.9-1.8-7.1-4.9-11.3-28.7-28.1-54.4-50.1-76.4-22-22-47.7-38.9-76.4-50.1-3.9-1.5-5.9-6-4.3-9.9 1.5-3.9 6-5.9 9.9-4.3 30.7 12.1 58.1 30.1 81.6 53.6 23.5 23.5 41.5 50.9 53.6 81.6 1.5 3.9-0.4 8.4-4.3 9.9-1 0.4-1.9 0.5-2.9 0.5zM695.4 432.2c-2.9 0-5.7-1.7-7-4.5-9-20.1-21.5-38.3-37.3-54.1-15.7-15.7-33.9-28.3-54.1-37.3-3.9-1.7-5.6-6.2-3.9-10.1 1.7-3.9 6.2-5.6 10.1-3.9 21.9 9.7 41.6 23.3 58.6 40.4 17.1 17.1 30.7 36.8 40.4 58.6 1.7 3.9 0 8.4-3.9 10.1-0.8 0.6-1.8 0.8-2.9 0.8zM408.3 795.6c-0.8 0-1.7-0.1-2.6-0.4-40.8-14.4-77.1-37.4-107.9-68.2-30.8-30.8-53.8-67.1-68.2-107.9-1.4-4 0.7-8.4 4.7-9.8 4-1.4 8.4 0.7 9.8 4.7 13.7 38.6 35.4 73 64.6 102.2 29.2 29.2 63.6 50.9 102.2 64.6 4 1.4 6.1 5.8 4.7 9.8-1.2 3-4.1 5-7.3 5zM417.6 745.7c-0.9 0-1.9-0.2-2.8-0.5-30.7-12.1-58.1-30.1-81.6-53.6-23.5-23.5-41.5-50.9-53.6-81.6-1.5-3.9 0.4-8.4 4.3-9.9s8.4 0.4 9.9 4.3c11.3 28.7 28.1 54.4 50.1 76.4s47.7 38.9 76.4 50.1c3.9 1.5 5.9 6 4.3 9.9-1.1 3.1-4 4.9-7 4.9zM425.8 701.9c-1 0-2.1-0.2-3.1-0.7-21.8-9.7-41.6-23.3-58.6-40.4-17.1-17.1-30.7-36.8-40.4-58.7-1.7-3.9 0-8.4 3.9-10.1 3.9-1.7 8.4 0 10.1 3.9 9 20.1 21.5 38.3 37.3 54.1 15.7 15.7 33.9 28.3 54.1 37.3 3.9 1.7 5.6 6.2 3.9 10.1-1.5 2.8-4.3 4.5-7.2 4.5z" fill="#F8F7F9" p-id="2836"></path></svg></div>
    </div>
    <div class="audio-info">
      <div class="audio-name">{{ currentSong.name }}</div>
      <div class="audio-singer">{{ currentSong.singer }}</div>
    </div>
    <div class="audio-control">
      <i class="el-icon-arrow-left icon-side"></i>
      <i class="el-icon-video-play icon-center" v-if="!playing" @click="handleToggle(true)"></i>
      <i class="el-icon-video-pause icon-center" v-else @click="handleToggle(false)"></i>
      <i class="el-icon-arrow-right icon-side"></i>
    </div>
    <div class="audio-progress"></div>
    <div class="audio-volume"></div>
    <div class="audio-tool"></div>
    <audio ref="audio" :src="currentSong.url" @loadstart="handleLoadStart" @loadeddata="handleLoadEnd"></audio>
  </div>
</template>

<script>
import { mapState, mapGetters, mapActions, mapMutations } from 'vuex';
import * as types from '../store/mutationType'
export default {
  name: "PlayerBar",
  data(){
    return {
    }
  },
  computed:{
    ...mapState(['playList','sequenceList','currentIndex','playing']),
    ...mapGetters(['currentSong'])
  },
  watch:{
    currentSong(newSong,oldSong){
      if(newSong.id===oldSong.id || !newSong.url || !newSong.id){
        return
      }
      this.$nextTick(()=>{
        const audio=this.$refs.audio
        audio.play()
      })
    },
    playing(newStatus){
      this.$nextTick(()=>{
        const audio=this.$refs.audio
        newStatus?audio.play():audio.pause()
      })
    }
  },
  methods:{
    ...mapActions(['selectPlay','pausePlay']),
    ...mapMutations({
      toSetPlayingState:types.SET_PLAYING_STATE
    }),
    handleToggle(status){
      if(this.currentSong.id){
        this.toSetPlayingState(status)
      }
    }
  }
}
</script>

<style lang="scss" scoped>
.player-bar{
  position: fixed;
  bottom: 0;
  width: 100%;
  height: 72px;
  background: white;
  box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1);
  display: flex;
  padding: 0 20px;
  justify-content: space-between;
  align-items: center;
  .audio-avatar{
    width: 60px;
    height: 60px;
    .avatar-image{
      width: 100%;
      border-radius: 4px;
    }
  }
  .audio-info{
    width: 200px;
    display: flex;
    flex-direction: column;
    .audio-name{
      font-weight: bolder;
      margin-bottom: 10px;
    }
    .audio-singer{
      font-size: small;
    }
  }
  .audio-control{
    display: flex;
    align-items: center;
    color: #7868e6;
    .icon-side{
      font-size: 30px;
      cursor: pointer;
    }
    .icon-center{
      font-size: 40px;
      cursor: pointer;
      margin: 0 10px;
    }
  }
}
</style>
